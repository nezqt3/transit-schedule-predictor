...
Перед запуском положите архив Docker-образа `ndtp-telemetry-emulator.tar` в эту папку: `emulator/ndtp-telemetry-emulator.tar`. Архив не хранится в Git и добавлен в `.gitignore`, поэтому его нужно передавать отдельно каждому участнику или скачивать из общего хранилища.
