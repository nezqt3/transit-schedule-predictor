...
> Этот каталог описывает неизменённый эмулятор организаторов.
...
Быстрый запуск вместе с Backend и Frontend: `make up-ndtp` из корня проекта.
Команда ищет архив образа в корне или здесь, загружает
[`official-demo-config.json`](official-demo-config.json) и проверяет, что оба
терминала появились в Backend. Обычный `config.json` остаётся пользовательским
и не перезаписывается. Без `POST /api/config` запущенный контейнер не шлёт
пакеты: после рестарта его конфигурация пуста.

`track` и одометр в ячейках означают расстояние, а не готовый маршрут.
Автогенерация строит синтетическое движение около Москвы; реальные маршруты и
остановки восстанавливаются по отдельным CSV с расписанием и GPS.

---

## 1. Запуск из Docker-образа

Для ручного запуска положите архив Docker-образа `ndtp-telemetry-emulator.tar` в эту папку. Команда `make up-ndtp` также найдёт архив в корне проекта. Архив не хранится в Git, поэтому его нужно передавать отдельно каждому участнику или скачивать из общего хранилища.
