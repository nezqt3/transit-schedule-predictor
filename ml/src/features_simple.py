"""Простой табличный билдер для notebooks/02_catboost.ipynb.

Канонический общий признакостроитель лежит в `ml/src/features.py`
(используется scripts/exp_*, инференсом и всеми экспериментами A-K).
Этот модуль сохранён, чтобы ноутбук-эксперимент оставался воспроизводимым.
"""
